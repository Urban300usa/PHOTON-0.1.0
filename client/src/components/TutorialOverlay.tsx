...
export default function TutorialOverlay() {
  const { isActive, currentStep, steps, currentStepData, next, prev, skip, complete } = useTutorial();
      ...
      switch (e.key) {
        ...
        case "Tab":
          ...
          if (focusableElements && focusableElements.length > 0) {
            ...
            if (e.shiftKey) {
              if (document.activeElement === firstElement || document.activeElement === tooltipRef.current) {
                lastElement.focus();
              } else {
              ...
              if (document.activeElement === lastElement) {
                firstElement.focus();
              } else if (document.activeElement === tooltipRef.current) {
                firstElement.focus();
              } else {
                const currentIndex = Array.from(focusableElements).indexOf(document.activeElement as Element);
    ...
    >
      ...
      >
        <CardContent className="p-4">
          ...
          <h3 className="font-semibold text-lg mb-2" data-testid="tutorial-title" id="tutorial-title">
            {currentStepData.title}
          </h3>
          <p className="text-sm text-muted-foreground mb-4" data-testid="tutorial-description" id="tutorial-description">
          ...
          <div className="flex justify-center gap-1.5 mt-4" role="progressbar" aria-valuenow={currentStep + 1} aria-valuemin={1} aria-valuemax={steps.length}>
            {steps.map((_, index) => (
              <div
                key={index}
